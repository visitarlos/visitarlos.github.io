...
def build_schedule(
    ...
    thanksgiving_break,
    xmas_last_schoolday,
    school_last_day=None,
    prev_xmas_school_return=None,
):
    even = year % 2 == 0
    end = date(year, 12, 31)
    S = {}
...
YEAR_DATA = {
    ...
    2026: dict(
        spring_break=(
            date(2026, 3, 23),
            date(2026, 3, 27),
        ),  # CONFIRMED: 2025-26 calendar
        thanksgiving_break=(
            date(2026, 11, 23),
            date(2026, 11, 27),
        ),  # CONFIRMED: 2026-27 calendar
        ...
        prev_xmas_school_return=date(2026, 1, 5),
    ),  # CONFIRMED: user
    ...
    2029: dict(
        spring_break=(date(2029, 3, 25), date(2029, 3, 29)),
    ...
    2030: dict(
        spring_break=(date(2030, 3, 18), date(2030, 3, 22)),
    ...
    2031: dict(
        spring_break=(date(2031, 3, 24), date(2031, 3, 28)),
...
def render(year, S, splits, all_years=()):
    ...
    nav_prev = (
        f'<a class="nav" href="visitation_calendar_{year-1}.html">&lsaquo;</a>'
        if year - 1 in all_years
        else '<span class="nav-ph"></span>'
    )
    nav_next = (
        f'<a class="nav" href="visitation_calendar_{year+1}.html">&rsaquo;</a>'
        if year + 1 in all_years
        else '<span class="nav-ph"></span>'
    )
